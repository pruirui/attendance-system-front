					...
					<el-main style="width: 50%; height:540px; padding-top: 5px; padding-right: 20px; padding-left: 20px; padding-bottom: 0px;" >
															...
															<el-tooltip :disabled="false" content="下午未打卡" placement="top" >
																<el-popover :visible="item.second_disabled" placement="top" :width="230">
																	...
																	<div style="width: 200px; height: 40px;">
																		<el-form :inline="true" >
																		    <el-form-item label="部门ID" style="width: 200px;" >
																    ...
																    <div style="text-align: right; margin: 0;padding-top: 10px;">
																		<el-button size="small" text @click="item.second_disabled = false">cancel</el-button>
																		<el-button size="small" type="primary" @click="item.second_disabled = false;handleClick(data.day,1)">confirm</el-button>
																    </div>
																    <template #reference>
														...
														<div >
															<el-tooltip :disabled="false" content="上午未打卡" placement="top" >
																<el-popover :visible="item.first_disabled" placement="top" :width="230">
																    ...
																    <el-input v-model="input" placeholder="请输入你的补卡理由" />
																    <div style="text-align: right; margin: 0;padding-top: 10px;">
																		<el-button size="small" text @click="item.first_disabled = false">cancel</el-button>
																		<el-button size="small" type="primary" @click="item.first_disabled = false;handleClick(data.day,0)">confirm</el-button>
																    </div>
																    <template #reference>
						...
						<div style="width: 460px; height: 30px; padding-right: 20px; padding-left: 0px;padding-bottom: 0;padding-top: 5;">
							<el-descriptions class="margin-top" title="打卡日历描述" :column="2"   border>
							    <template #extra><el-button type="primary" size="small" @click="handleLeave">请假申请</el-button></template>
							    <el-descriptions-item>
									<template #label> <div class="cell-item" style="text-align: center;"><!-- <el-icon  :style="iconStyle"><Check /></el-icon> -->✓</div></template>
									✓代表已经打卡
							    </el-descriptions-item>
							    <el-descriptions-item>
							      <template #label><div class="cell-item" style="text-align: center;"><!-- <el-icon :style="iconStyle"><Close /> </el-icon> -->✗</div> </template>
							      ✗代表没有打卡
							    </el-descriptions-item>
							    <el-descriptions-item>
							      <template #label> <div class="cell-item" style="text-align: center;"> <!-- <el-icon :style="iconStyle"><Notification /></el-icon> --> 描述 </div></template>
...
getDepartmentByUid(uid).then((res) => {
	...
	if (data.code == 1) {
		...
		for(let i=0;i<len;i++){
			let info=ref({});
			info.value["label"]='部门'+data.data[i].departmentid;
...
const handleClick = (date, code) => {
	usermakeUpClock(uid, departmentid.value, date, code, input.value).then((res) => {
		console.log(res);
	});
	input.value='';
}
